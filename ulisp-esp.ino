...
// Includes
#ifdef lisplibrary // Kaef LispLibrary
...
int compactimage (object **arg) {
    ...
    while (firstfree < obj) {
        if (marked(obj)) {
            /* */
            // Kaef: Debugoutput -- I wanted a percent gauge here, but it's not known how many cons need to be processed...
            //Serial.print(" "); Serial.print((int)(obj - firstfree));
            //if(++count > 15) { Serial.println(""); count = 0; }
            pfstring(PSTR("."), pserial); if (++count > 60) {
                count = 0;
                pln(pserial);
            }
            // */
            ...
            unmark(obj);
            movepointer(idxMaxUsedCon, obj, firstfree);
            if (GlobalEnv == obj) GlobalEnv = firstfree;
            if (GCStack == obj) GCStack = firstfree;
            if (*arg == obj) *arg = firstfree;
            //Serial.println(obj - firstfree);
            while (marked(firstfree)) firstfree++;
        }
        obj--;
    }
...
int saveimage (object *arg) {
    ...
    if (bytesneeded > EEPROMSIZE) {
        ...
        pint(imagesize, pserial); pfstring(PSTR(" ("), pserial);
        pint(bytesneeded, pserial); pfstring(PSTR(" Bytes needed /"), pserial);
        pint(EEPROMSIZE, pserial); pfstring(PSTR(" Bytes free)"), pserial); pln(pserial);
        GCStack = NULL;
        longjmp(exception, 1);
    }
...
void printFreeHeap () {
    pfstring(PSTR("  Free Heap: "), pserial); pint(esp_get_free_heap_size(), pserial); pln(pserial);
}
// Setup
