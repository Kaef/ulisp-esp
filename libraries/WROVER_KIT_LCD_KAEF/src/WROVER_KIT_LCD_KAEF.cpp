...
void  WROVER_KIT_LCD_KAEF::writeInitData(const uint8_t * data){
    uint8_t cmd, len, i;
    while(true){
        cmd = *data++;
        if(!cmd){ //END
...
uint32_t WROVER_KIT_LCD_KAEF::readId() {
    uint32_t freq = _freq;
    if(_freq > 24000000){
        _freq = 24000000;
    }
    ...
    tftSPI->write(0x10);
    writeCommand(0x04);
    uint32_t r = tftSPI->transfer(0) << 16;
    r |= tftSPI->transfer(0) << 8;
    r |= tftSPI->transfer(0);
    endWrite();
    _freq = freq;
    return r;
...
uint16_t WROVER_KIT_LCD_KAEF::color565(uint8_t r, uint8_t g, uint8_t b) {
    return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | ((b & 0xF8) >> 3);
}

WROVER_KIT_LCD_KAEF::WROVER_KIT_LCD_KAEF(SPIClass useSPI) : Adafruit_GFX(WROVER_WIDTH, WROVER_HEIGHT) {
    _id   = 0;
    _freq = WROVER_DEFAULT_FREQ;
    tftSPI= new SPIClass(useSPI);
}

void WROVER_KIT_LCD_KAEF::startWrite(void){
    tftSPI->beginTransaction(SPISettings(_freq, MSBFIRST, SPI_MODE0));
    WROVER_CS_LOW();
}

void WROVER_KIT_LCD_KAEF::endWrite(void){
    WROVER_CS_HIGH();
    tftSPI->endTransaction();
}

void WROVER_KIT_LCD_KAEF::writeCommand(uint8_t cmd){
    WROVER_DC_LOW();
    tftSPI->write(cmd);
    WROVER_DC_HIGH();
}

void WROVER_KIT_LCD_KAEF::begin(){
    _width  = WROVER_WIDTH;
    ...
    tftSPI->begin(WROVER_SCLK, WROVER_MISO, WROVER_MOSI, -1);
...
void WROVER_KIT_LCD_KAEF::setRotation(uint8_t m) {
...
void WROVER_KIT_LCD_KAEF::invertDisplay(boolean i) {
    startWrite();
    writeCommand(i ? WROVER_INVON : WROVER_INVOFF);
    endWrite();
}

void WROVER_KIT_LCD_KAEF::scrollTo(uint16_t y) {
    startWrite();
...
void WROVER_KIT_LCD_KAEF::setupScrollArea(uint16_t tfa, uint16_t bfa) {
  startWrite();
  writeCommand(WROVER_VSCRDEF); // Vertical scroll definition
  tftSPI->write(tfa >> 8);
  tftSPI->write(tfa);
  tftSPI->write((WROVER_HEIGHT - tfa - bfa) >> 8);
  tftSPI->write(WROVER_HEIGHT - tfa - bfa);
  tftSPI->write(bfa >> 8);
  tftSPI->write(bfa);
  endWrite();
}
...
void WROVER_KIT_LCD_KAEF::setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
...
void WROVER_KIT_LCD_KAEF::startBitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
    startWrite();
    writeCommand(WROVER_MADCTL);
    if(_id){
        tftSPI->write(st7789v_rotations[rotation].bmpctl);
    } else {
        tftSPI->write(ili9341_rotations[rotation].bmpctl);
    }
    endWrite();
    startWrite();
...
void WROVER_KIT_LCD_KAEF::endBitmap() {
    endWrite();
    startWrite();
    writeCommand(WROVER_MADCTL);
    if(_id){
        tftSPI->write(st7789v_rotations[rotation].madctl);
    } else {
        tftSPI->write(ili9341_rotations[rotation].madctl);
    }
    endWrite();
}

void WROVER_KIT_LCD_KAEF::pushColor(uint16_t color) {
    startWrite();
    tftSPI->write16(color);
    endWrite();
}

void WROVER_KIT_LCD_KAEF::writePixel(uint16_t color){
    tftSPI->write16(color);
}

void WROVER_KIT_LCD_KAEF::writePixels(uint16_t * colors, uint32_t len){
    tftSPI->writePixels((uint8_t*)colors , len * 2);
}

void WROVER_KIT_LCD_KAEF::writeColor(uint16_t color, uint32_t len){
...
void WROVER_KIT_LCD_KAEF::writePixel(int16_t x, int16_t y, uint16_t color) {
    if((x < 0) ||(x >= _width) || (y < 0) || (y >= _height)) return;
    setAddrWindow(x,y,1,1);
    writePixel(color);
}

void WROVER_KIT_LCD_KAEF::writeFillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color){
...
void WROVER_KIT_LCD_KAEF::writeFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color){
    writeFillRect(x, y, 1, h, color);
}

void WROVER_KIT_LCD_KAEF::writeFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color){
    writeFillRect(x, y, w, 1, color);
}

uint8_t WROVER_KIT_LCD_KAEF::readcommand8(uint8_t c, uint8_t index) {
    uint32_t freq = _freq;
    if(_freq > 24000000){
        _freq = 24000000;
    }
    ...
    tftSPI->write(0x10 + index);
    writeCommand(c);
    uint8_t r = tftSPI->transfer(0);
    endWrite();
    _freq = freq;
    return r;
}

void WROVER_KIT_LCD_KAEF::drawPixel(int16_t x, int16_t y, uint16_t color){
    startWrite();
    writePixel(x, y, color);
    endWrite();
}

void WROVER_KIT_LCD_KAEF::drawFastVLine(int16_t x, int16_t y,
...
void WROVER_KIT_LCD_KAEF::drawFastHLine(int16_t x, int16_t y,
...
void WROVER_KIT_LCD_KAEF::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
        uint16_t color) {
    startWrite();
...
void WROVER_KIT_LCD_KAEF::drawBitmap(int16_t x, int16_t y, int16_t w, int16_t h, const uint16_t *pcolors) {
    startWrite();
    setAddrWindow(x, y, w, h);
...
void WROVER_KIT_LCD_KAEF::bmpSkipPixels(fs::File &file, uint8_t bitsPerPixel, size_t len){
    size_t bytesToSkip = (len * bitsPerPixel) / 8;
    file.seek(bytesToSkip, SeekCur);
}

void WROVER_KIT_LCD_KAEF::bmpAddPixels(fs::File &file, uint8_t bitsPerPixel, size_t len){
...
void WROVER_KIT_LCD_KAEF::drawBmpFile(fs::FS &fs, const char * path, uint16_t x, uint16_t y, uint16_t maxWidth, uint16_t maxHeight, uint16_t offX, uint16_t offY){
...
void WROVER_KIT_LCD_KAEF::drawJpg(const uint8_t * jpg_data, size_t jpg_len, uint16_t x, uint16_t y, uint16_t maxWidth, uint16_t maxHeight, uint16_t offX, uint16_t offY, jpeg_div_t scale){
    if((x + maxWidth) > width() || (y + maxHeight) > height()){
        log_e("Bad dimensions given");
        return;
    }
...
void WROVER_KIT_LCD_KAEF::drawJpgFile(fs::FS &fs, const char * path, uint16_t x, uint16_t y, uint16_t maxWidth, uint16_t maxHeight, uint16_t offX, uint16_t offY, jpeg_div_t scale){
    if((x + maxWidth) > width() || (y + maxHeight) > height()){
        log_e("Bad dimensions given");
        return;
    }
